...
import type { LoginWithIdTokenMutation } from '../generated/graphql';
...
export async function refreshAppToken(): Promise<boolean> {
  try {
    ...
    const { data } = await apolloClient.mutate<LoginWithIdTokenMutation>({ mutation: MUTATION_LOGIN_WITH_ID_TOKEN, variables: { idToken } });
